...
def plot_full_song_constellation(best_song, best_offset, clip_len_frames):
    ...
    all_times_sec   = librosa.frames_to_time(all_times,   sr=SR, hop_length=HOP)
    clip_start_sec  = librosa.frames_to_time(best_offset,              sr=SR, hop_length=HOP)
    clip_end_sec    = librosa.frames_to_time(best_offset + clip_len_frames, sr=SR, hop_length=HOP)

    fig, (ax,) = dark_fig(11, 3.8)
    ax.scatter(all_times_sec, all_freqs, color=TEAL_PT, s=1.5,
               linewidths=0, alpha=0.6)
    ax.axvspan(clip_start_sec, clip_end_sec, color=AMBER, alpha=0.12, zorder=3)
    ax.axvline(clip_start_sec, color=AMBER, linewidth=1.2, linestyle='--', alpha=0.9)
    ax.axvline(clip_end_sec,   color=AMBER, linewidth=1.2, linestyle='--', alpha=0.9)
